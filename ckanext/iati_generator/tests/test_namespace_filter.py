...
class TestNamespaceFilter:
    ...
    def test_namespace_filter_with_pagination(self, setup_data_with_namespaces):
        """Test namespace filter works correctly with pagination."""
        setup = setup_data_with_namespaces

        # Create multiple IATI files with same namespace
        create_iati_file(
            resource_id=setup.res1["id"],
            namespace="iati-xml",
            file_type=IATIFileTypes.ORGANIZATION_MAIN_FILE,
        ...
        result_page1 = self._call_action_as_user(
            "iati_file_list",
            setup.sysadmin["name"],
            namespace="iati-xml",
            start=0,
        ...
        result_page2 = self._call_action_as_user(
            "iati_file_list",
            setup.sysadmin["name"],
            namespace="iati-xml",
            start=2,
        ...
        # Total count should be 3 for both pages
        assert result_page1["count"] == 3
        assert result_page2["count"] == 3

        # Page 1 should have 2 results, page 2 should have 1
        assert len(result_page1["results"]) == 2
        assert len(result_page2["results"]) == 1

        # All results should have iati-xml namespace
        for item in result_page1["results"] + result_page2["results"]:
            assert item["namespace"] == "iati-xml"

    def test_namespace_filter_combined_with_other_filters(self, setup_data_with_namespaces):
        """Test namespace filter works in combination with other filters."""
        setup = setup_data_with_namespaces

        # Create IATI files with different combinations
        create_iati_file(
            resource_id=setup.res1["id"],
            namespace="iati-xml",
            file_type=IATIFileTypes.ORGANIZATION_MAIN_FILE,
        ...
        # Filter by namespace AND file_type
        result = self._call_action_as_user(
            "iati_file_list",
            setup.sysadmin["name"],
            namespace="iati-xml",
        ...
        assert result["results"][0]["namespace"] == "iati-xml"
        assert result["results"][0]["file_type"] == "ORGANIZATION_MAIN_FILE"

        # Filter by namespace AND valid
        result2 = self._call_action_as_user(
            "iati_file_list",
            setup.sysadmin["name"],
            namespace="iati-xml",
            valid="true"
        ...
        assert result2["results"][0]["namespace"] == "iati-xml"
        assert result2["results"][0]["is_valid"] is True
